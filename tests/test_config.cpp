...
//各种STL类型
//创建int类型配置对象
yancy::ConfigVar<int>::ptr g_int_value_config=
    yancy::Config::Lookup("system.port",(int)8080,"system port");   

//创建float类型配置对象
yancy::ConfigVar<float>::ptr g_int_valuex_config=
    yancy::Config::Lookup("system.port",(float)8080,"system port"); 

//创建float类型配置对象
yancy::ConfigVar<float>::ptr g_float_value_config=
    yancy::Config::Lookup("system.value",(float)10.2f,"system value");

//创建vector<int>类型配置对象
yancy::ConfigVar<std::vector<int>>::ptr g_int_vec_value_config=
    yancy::Config::Lookup("system.int_vec",std::vector<int>{1,2},"system int vec");

//创建list<int>类型配置对象
yancy::ConfigVar<std::list<int>>::ptr g_int_list_value_config=
    yancy::Config::Lookup("system.int_list",std::list<int>{1,2},"system int list");

//创建set<int>类型配置对象
yancy::ConfigVar<std::set<int>>::ptr g_int_set_value_config=
    yancy::Config::Lookup("system.int_set",std::set<int>{1,2},"system int set");

//创建unordered_set<int>类型配置对象
yancy::ConfigVar<std::unordered_set<int>>::ptr g_int_uset_value_config=
    yancy::Config::Lookup("system.int_uset",std::unordered_set<int>{1,2},"system int uset");

//创建map<std::string,int>类型配置对象
yancy::ConfigVar<std::map<std::string,int>>::ptr g_str_int_map_value_config=
    yancy::Config::Lookup("system.str_int_map",std::map<std::string,int>{{"k",2}},"system str int map");
...
//创建person类配置对象
yancy::ConfigVar<Person>::ptr g_person=
    yancy::Config::Lookup("class.person",Person(),"system person");  

//创建std::map<std::string,Person>类配置对象
yancy::ConfigVar<std::map<std::string,Person>>::ptr g_person_map=
    yancy::Config::Lookup("class.map",std::map<std::string,Person>(),"system person");  

//创建std::map<std::string,Person>类配置对象
yancy::ConfigVar<std::map<std::string,std::vector<Person>>>::ptr g_person_vec_map=
    yancy::Config::Lookup("class.vec_map",std::map<std::string,std::vector<Person>>(),"system person");  
...
void test_log()
{
    ...
    std::cout<<"======================"<<std::endl;
    std::cout<<yancy::LoggerMgr::GetInstance()->toYamlString()<<std::endl;  //事件变更之后******用日志器里的yaml::node解析log.txt并打印————YAML：：Node是静态全局变量
    std::cout<<"======================"<<std::endl;
...
void test_loadconf()
{
    yancy::Config::LoadFromConfDir("conf");
}
...
int main(int argc,char ** argv)
{
    ...
    test_log();
    // yancy::EnvMgr::GetInstance()->init(argc,argv);
    // test_loadconf();
    // std::cout<<"============"<<std::endl;
    // test_loadconf();
    // return 0;
