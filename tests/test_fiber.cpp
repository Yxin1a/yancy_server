...
{
    YANCY_LOG_INFO(g_logger)<<"run_in_fiber begin";
    yancy::Fiber::YieldToHold();    //将当前协程切换到后台,并设置为HOLD状态(切换到主协程)
    YANCY_LOG_INFO(g_logger)<<"run_in_fiber end";
    yancy::Fiber::YieldToHold();    //将当前协程切换到后台,并设置为HOLD状态(切换到主协程)
}

void test_fiber()   //主协程函数
{
    YANCY_LOG_INFO(g_logger)<<"main begin -1";
    {
        yancy::Fiber::GetThis();    //返回当前所在的协程,没有则创建新的主协程
        YANCY_LOG_INFO(g_logger)<<"main begin";
        yancy::Fiber::ptr fiber(new yancy::Fiber(run_in_fiber));    //new创建子协程
        fiber->swapIn();    //将当前协程切换到运行状态，并将主协程切换到后台(切换到子协程)
        YANCY_LOG_INFO(g_logger)<<"main after swapIn";
        fiber->swapIn();    //将当前协程切换到运行状态，并将主协程切换到后台(切换到子协程)
        YANCY_LOG_INFO(g_logger)<<"main after end";
        fiber->swapIn();    //将当前协程切换到运行状态，并将主协程切换到后台(切换到子协程释放————手动)
    }
    YANCY_LOG_INFO(g_logger)<<"main after end2";
}
