...
namespace yancy
{
    ...
    Socket::ptr Socket::CreateUDP(yancy::Address::ptr address)  //创建UDP Socket(满足地址类型)
    {
    ...
    bool Socket::bind(const Address::ptr addr)  //服务器绑定IP地址
    {
        ...
        //     else
        //     {
        //         yancy::FSUtil::Unlink(uaddr->getPath(), true);
        //     }
        // }
    ...
    {
        return IOManager::GetThis()->cancelEvent(m_sock, yancy::IOManager::READ);   //回调函数默认为空
    }
    
    bool Socket::cancelWrite() //取消写(唤醒执行)
    {
        return IOManager::GetThis()->cancelEvent(m_sock, yancy::IOManager::WRITE);  //回调函数默认为空
    }
