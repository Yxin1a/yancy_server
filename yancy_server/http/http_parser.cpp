...
namespace yancy
{
    namespace http
    {
        ...
        static yancy::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
            yancy::Config::Lookup("http.request.buffer_size"
                        ,(uint64_t)(4 * 1024), "http request buffer size");

        //限制请求消息体一个字段(一次发送的数据大小)为64M————超出则数据有问题
        static yancy::ConfigVar<uint64_t>::ptr g_http_request_max_body_size =
            yancy::Config::Lookup("http.request.max_body_size"
                        ,(uint64_t)(64 * 1024 * 1024), "http request max body size");

        //限制响应头部一个字段(一次发送的数据大小)为4K————超出则数据有问题
        static yancy::ConfigVar<uint64_t>::ptr g_http_response_buffer_size =
            yancy::Config::Lookup("http.response.buffer_size"
                        ,(uint64_t)(4 * 1024), "http response buffer size");

        //限制响应消息体一个字段(一次发送的数据大小)为64M————超出则数据有问题
        static yancy::ConfigVar<uint64_t>::ptr g_http_response_max_body_size =
            yancy::Config::Lookup("http.response.max_body_size"
        ...
        */
        ...
        HttpRequestParser::HttpRequestParser()  //初始化    m_error m_data m_parser
            :m_error(0)
        {
            m_data.reset(new yancy::http::HttpRequest);
        ...
        */
        ...
        HttpResponseParser::HttpResponseParser()    //初始化    m_error m_data m_parser
            :m_error(0)
        {
            m_data.reset(new yancy::http::HttpResponse);
