...
namespace yancy
{
    namespace http
    {
        ...
        int32_t FunctionServlet::handle(yancy::http::HttpRequest::ptr request   //处理请求(回调函数)
        ...
        {
            m_default.reset(new NotFoundServlet("yancy/1.0"));
        }

        int32_t ServletDispatch::handle(yancy::http::HttpRequest::ptr request   //处理请求(决定具体请求哪个Servlet)
                    , yancy::http::HttpResponse::ptr response
                    , yancy::http::HttpSession::ptr session)
        {
        ...
        int32_t NotFoundServlet::handle(yancy::http::HttpRequest::ptr request   //处理(访问错误)
                        , yancy::http::HttpResponse::ptr response
                        , yancy::http::HttpSession::ptr session)
        {
            response->setStatus(yancy::http::HttpStatus::NOT_FOUND);
            response->setHeader("Server", "yancy/1.0.0");
