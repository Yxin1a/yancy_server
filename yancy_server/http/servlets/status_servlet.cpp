...
namespace yancy
{
     ...
     *      是否处理成功
     */
    int32_t StatusServlet::handle(yancy::http::HttpRequest::ptr request
    ...
    #define XX(key) \
        ...
        std::stringstream ss;
        ss << "===================================================" << std::endl;
        XX("server_version") << "yancy/1.0.0" << std::endl;
        ...
        ss << "===================================================" << std::endl;
        XX("fibers") << yancy::Fiber::TotalFibers() << std::endl;
        ss << "===================================================" << std::endl;
        ss << "<Logger>" << std::endl;
        ss << yancy::LoggerMgr::GetInstance()->toYamlString() << std::endl;
        ss << "===================================================" << std::endl;
        ss << "<Woker>" << std::endl;
        yancy::WorkerMgr::GetInstance()->dump(ss) << std::endl;

        std::map<std::string, std::vector<TcpServer::ptr> > servers;
        yancy::Application::GetInstance()->listAllServer(servers);
        ss << "===================================================" << std::endl;
                ...
                it != servers.end(); ++it)
        {
                    ...
                    iit != it->second.end(); ++iit)
            {
                ...
                if(!hs)
                {
                    hs = std::dynamic_pointer_cast<yancy::http::HttpServer>(*iit);
                }
