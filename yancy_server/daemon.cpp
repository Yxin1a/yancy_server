...
namespace yancy
{
    ...
    static yancy::ConfigVar<uint32_t>::ptr g_daemon_restart_interval
        = yancy::Config::Lookup("daemon.restart_interval", (uint32_t)5, "daemon restart interval");
    ...
    std::string ProcessInfo::toString() const   //返回进程生命周期字符串
    {
        ...
        << " main_id=" << main_id
        << " parent_start_time=" << yancy::Time2Str(parent_start_time)
        << " main_start_time=" << yancy::Time2Str(main_start_time)
