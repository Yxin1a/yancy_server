...
namespace yancy
{
    ...
    static yancy::ConfigVar<int>::ptr g_tcp_connect_timeout=
        yancy::Config::Lookup("tcp.connect.timeout",5000,"tcp connect timeout");
    ...
    */
    ...
    {   
        //不用hook
        if(!yancy::t_hook_enable)   //当前线程不采用hook
        {   //forward(完美转发)按照参数原来的类型转发到另一个函数
            return fun(fd,std::forward<Args>(args)...); //什么都不做，把原函数返回回去
        }
        
        yancy::FdCtx::ptr ctx=yancy::FdMgr::GetInstance()->get(fd);  //获取/创建文件句柄类FdCtx
...
retry:  //失败继续读取标签
        ...
        {
            yancy::IOManager* iom=yancy::IOManager::GetThis();  //IO
            yancy::Timer::ptr timer;
            ...
            if(to!=(uint64_t)-1)    //没有超时,要加条件定时器
            {
                ...
                {   //添加条件定时器(如果对应的共享指针存在，执行定时器回调函数)
                    ...
                    iom->cancelEvent(fd,(yancy::IOManager::Event)(event));  //超时，则执行函数
                },winfo);
            }
            ...
            {
                yancy::Fiber::YieldToHold();   //将当前协程切换到后台，并将主协程切换出来,并设置为HOLD状态
                if (timer)
                {
                    timer->cancel();    //删除定时器
                }
...
extern "C"  //extern说明 此变量/函数是在别处定义的，要在此处引用
{
    ...
    {
        if(!yancy::t_hook_enable)
        {
            return sleep_f(seconds);    //返回原来函数
        }

        yancy::Fiber::ptr fiber=yancy::Fiber::GetThis();    //返回当前协程
        yancy::IOManager* iom=yancy::IOManager::GetThis();  //返回当前iomanager
        //iom->addTimer(seconds*1000,std::bind(&yancy::IOManager::schedule,iom,fiber));   //给当前的iomananger加定时器
        iom->addTimer(seconds*1000,std::bind((void(yancy::Scheduler::*) //(void(yancy::Scheduler::*)(yancy::Fiber::ptr,int thread))定义bind模板方法
                (yancy::Fiber::ptr,int thread))& yancy::IOManager::schedule //thread是函数有默认值
                ,iom,fiber,-1));
        yancy::Fiber::YieldToHold();    //切换到主协程
        return 0;
    }
    ...
    int usleep(useconds_t usec) //重新编写和sleep差不多的函数—————微秒级
    {
        ...
        yancy::Fiber::ptr fiber=yancy::Fiber::GetThis();    //返回当前协程
        yancy::IOManager* iom=yancy::IOManager::GetThis();  //返回当前iomanager
        //iom->addTimer(usec/1000,std::bind(&yancy::IOManager::schedule,iom,fiber));   //给当前的iomananger加定时器
        iom->addTimer(usec/1000,std::bind((void(yancy::Scheduler::*)    //(void(yancy::Scheduler::*)(yancy::Fiber::ptr,int thread))定义bind模板方法
                (yancy::Fiber::ptr,int thread))& yancy::IOManager::schedule //thread是函数有默认值
                ,iom,fiber,-1));
        yancy::Fiber::YieldToHold();    //切换到主协程
        return 0;
    ...
    int nanosleep(const struct timespec *req,struct timespec *rem)  //重新编写和sleep差不多的函数————毫秒级
    {
        ...
        yancy::IOManager* iom=yancy::IOManager::GetThis();  //返回当前iomanager
        //iom->addTimer(seconds*1000,std::bind(&yancy::IOManager::schedule,iom,fiber));   //给当前的iomananger加定时器
        iom->addTimer(timeout_ms,std::bind((void(yancy::Scheduler::*)   //(void(yancy::Scheduler::*)(yancy::Fiber::ptr,int thread))定义bind模板方法
                (yancy::Fiber::ptr,int thread))& yancy::IOManager::schedule //thread是函数有默认值
                ,iom,fiber,-1));
        yancy::Fiber::YieldToHold();    //切换到主协程
        return 0;
    ...
    int socket(int domain,int type,int protocol)    //重新编写socket，覆盖原本的系统函数
    {
        ...
        yancy::FdMgr::GetInstance()->get(fd,true);  //获取文件句柄类FdCtx————自动创建，并插入容器
        return fd;
    }
    ...
    int connect_with_timeout(int fd, const struct sockaddr* addr, socklen_t addrlen, uint64_t timeout_ms)   //重新编写和connect差不多的函数(增加了超时变量)
    {
        ...
        yancy::FdCtx::ptr ctx = yancy::FdMgr::GetInstance()->get(fd);   //获取文件句柄类FdCtx————自动创建，并插入容器
        ...
        yancy::IOManager* iom = yancy::IOManager::GetThis();
        yancy::Timer::ptr timer;
        ...
        if(timeout_ms != (uint64_t)-1)  //没有超时，加条件定时器
        {
            ...
            {   //添加条件定时器(如果对应的共享指针存在，执行定时器回调函数)
                ...
                t->cancelled = ETIMEDOUT;   //ETIMEDOUT超时
                iom->cancelEvent(fd, yancy::IOManager::WRITE);  //超时，则执行函数
            }, winfo);
        }

        int rt = iom->addEvent(fd, yancy::IOManager::WRITE);    //添加IOmanager事件
        if(rt == 0) //添加成功
        {
            yancy::Fiber::YieldToHold();    //将当前协程切换到后台，并将主协程切换出来
            if(timer)
            {
                timer->cancel();    //删除定时器
            }
    ...
    {
        return connect_with_timeout(sockfd, addr, addrlen, yancy::s_connect_timeout);   //和connect差不多的函数(增加了超时变量)
    }

    int accept(int s,struct sockaddr* addr,socklen_t* addrlen)  //重新编写accept，覆盖原本的系统函数
    {
        int fd=do_io(s,accept_f,"accept",yancy::IOManager::READ,SO_RCVTIMEO,addr,addrlen);  //有超时时间
        if(fd>=0)
        {
            yancy::FdMgr::GetInstance()->get(fd,true);
        }
        return fd;
    }
    ...
    {
        return do_io(fd,read_f,"read",yancy::IOManager::READ,SO_RCVTIMEO,buf,count);    //SO_RCVTIMEO设置socket接收数据超时时间
    }

    ssize_t readv(int fd, const struct iovec *iov, int iovcnt)  //重新编写readv，覆盖原本的系统函数
    {
        return do_io(fd,readv_f,"readv",yancy::IOManager::READ,SO_RCVTIMEO,iov,iovcnt);
    }

    ssize_t recv(int sockfd, void *buf, size_t len, int flags)  //重新编写recv，覆盖原本的系统函数
    {
        return do_io(sockfd,recv_f,"recv",yancy::IOManager::READ,SO_RCVTIMEO,buf,len,flags);
    }
    ...
    {
        return do_io(sockfd,recvfrom_f,"recvfrom",yancy::IOManager::READ,SO_RCVTIMEO,buf,len,flags,src_addr,addrlen);
    }

    ssize_t recvmsg(int sockfd, struct msghdr *msg, int flags)  //重新编写recvmsg，覆盖原本的系统函数
    {
        return do_io(sockfd,recvmsg_f,"recvmsg",yancy::IOManager::READ,SO_RCVTIMEO,msg,flags);
    }

    //write
    ssize_t write(int fd, const void *buf, size_t count)    //重新编写write，覆盖原本的系统函数
    {
        return do_io(fd,write_f,"write",yancy::IOManager::WRITE,SO_SNDTIMEO,buf,count); //SO_SNDTIMEO设置socket发送数据超时时间
    }

    ssize_t writev(int fd, const struct iovec *iov, int iovcnt) //重新编写writev，覆盖原本的系统函数
    {
        return do_io(fd,writev_f,"writev",yancy::IOManager::WRITE,SO_SNDTIMEO,iov,iovcnt);
    }

    ssize_t send(int sockfd, const void *buf, size_t len, int flags)    //重新编写send，覆盖原本的系统函数
    {
        return do_io(sockfd,send_f,"send",yancy::IOManager::WRITE,SO_SNDTIMEO,buf,len,flags);
    }
    ...
    {
        return do_io(sockfd,sendto_f,"sendto",yancy::IOManager::WRITE,SO_SNDTIMEO,buf,len,flags,dest_addr,addrlen);
    }

    ssize_t sendmsg(int sockfd, const struct msghdr *msg, int flags)    //重新编写sendmsg，覆盖原本的系统函数
    {
        return do_io(sockfd,sendmsg_f,"sendmsg",yancy::IOManager::WRITE,SO_SNDTIMEO,msg,flags);
    }
    ...
    int close(int fd)   //重新编写write，覆盖原本的系统函数
    {
        ...
        if(ctx)
        {
            auto iom=yancy::IOManager::GetThis();
            if(iom)
            {
                iom->cancelAll(fd); //执行所有事件
            }
            yancy::FdMgr::GetInstance()->del(fd);   //删除文件句柄类
        }
    }
    ...
    int ioctl(int d, unsigned long int request, ...)  //重新编写ioctl，覆盖原本的系统函数(ioctl控制接口函数)
    {
        ...
        if(FIONBIO == request)
        {
            ...
            yancy::FdCtx::ptr ctx = yancy::FdMgr::GetInstance()->get(d);    //获取文件句柄类FdCtx————自动创建，并插入容器
            if(!ctx || ctx->isClose() || !ctx->isSocket())  //不存在，已关闭，不是socket
            {
    ...
    int setsockopt(int sockfd, int level, int optname,const void *optval, socklen_t optlen) //为网络套接字设置选项值，比如：允许重用地址、网络超时 
    {
        ...
        if(level == SOL_SOCKET)
        {
            ...
            {
                yancy::FdCtx::ptr ctx = yancy::FdMgr::GetInstance()->get(sockfd);   //获取文件句柄类FdCtx————自动创建，并插入容器
                if(ctx)
                {
